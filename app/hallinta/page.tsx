...
import { saveGames, clearAllGames, getGames, getPlayers, savePlayer, deletePlayer, type Player } from "@/lib/storage";
...
export default function HallintaPage() {
  ...
  const handleImport = useCallback(async () => {
    ...
    setImportStatus({ type: "success", message: `Tuotu ${saved.length} kotipeliä!${skipped > 0 ? ` (${skipped} duplikaattia ohitettu)` : ""}` });
    setParsedGames([]);
  ...
  const handleAddPlayers = useCallback(async (e: React.FormEvent) => {
    ...
    if (newPlayers.length > 0) {
      ...
      setImportStatus({ type: "success", message: `Lisätty ${newPlayers.length} pelaajaa${skipped > 0 ? ` (${skipped} duplikaattia ohitettu)` : ""}` });
    }
  }, [playerNames, players]);
  ...
  return (
    <Box sx={{ minHeight: "100vh", bgcolor: "background.default" }}>
      ...
      <Container maxWidth="md" sx={{ py: 4 }}>
        <Stack gap={3}>
          {importStatus && <Alert severity={importStatus.type} onClose={() => setImportStatus(null)}>{importStatus.message}</Alert>}
          ...
          <Card>
            <CardContent>
              ...
              <form onSubmit={handleAddPlayers}>
                <TextField fullWidth multiline rows={3} value={playerNames} onChange={(e) => setPlayerNames(e.target.value)} placeholder="Lisää pelaajia (yksi per rivi)" size="small" sx={{ mb: 2 }} />
                <Button type="submit" variant="contained" size="small">Lisää pelaajat</Button>
              </form>

              {players.length > 0 && (
                <Stack direction="row" flexWrap="wrap" gap={1} mt={2}>
                  {players.map((player) => (
                    <Chip key={player.id} label={player.name} onDelete={() => handleDeletePlayer(player.id)} deleteIcon={<CloseIcon />} />
                  ))}
                </Stack>
              )}
            </CardContent>
            ...
            <CardContent>
              <Stack direction="row" alignItems="center" justifyContent="space-between" mb={2}>
                ...
                {existingGamesCount > 0 && (
                  <Button color="error" size="small" startIcon={<DeleteIcon />} onClick={handleClearAll}>
              ...
              <Box
                ...
                onDrop={handleDrop}
                sx={{ border: 2, borderStyle: "dashed", borderColor: isDragging ? "primary.main" : "grey.300", borderRadius: 2, p: 4, textAlign: "center", bgcolor: isDragging ? "primary.50" : "transparent" }}
              >
                <Typography color="text.secondary" gutterBottom>Vedä Excel-tiedosto tähän tai</Typography>
                <Button variant="contained" component="label" startIcon={<UploadFileIcon />}>
                  Valitse tiedosto
                  <input type="file" hidden accept=".xlsx,.xls" onChange={(e) => e.target.files?.[0] && handleFile(e.target.files[0])} />
                </Button>
              </Box>
            </CardContent>
          </Card>
            ...
            <Card>
              <CardContent>
                <Stack direction="row" alignItems="center" justifyContent="space-between" mb={2}>
                  ...
                  <Button variant="contained" color="success" onClick={handleImport}>Tuo pelit</Button>
                </Stack>
                  ...
                  <Table size="small">
                    ...
                    <TableBody>
                      {parsedGames.map((game, index) => (
                        <TableRow key={index}>
                          <TableCell><Chip label={game.division} size="small" color="primary" variant="outlined" /></TableCell>
                          <TableCell>{game.opponent}</TableCell>
                          <TableCell>{formatDate(game.date)}</TableCell>
                          <TableCell sx={{ fontWeight: "bold", color: "primary.main" }}>{game.time}</TableCell>
